...
impl<C: Client + 'static> JavaScript<C> {
    ...
    fn new_in(_client: C, path: PathBuf) -> Result<Box<dyn Engine>> {
        let data = fs::read_to_string(&path).context("could not read package.json")?;

        let pkg = match json::parse(&data).context("invalid package.json")? {
            JsonValue::Object(pkg) => pkg,
            _ => bail!("package.json is not a JSON object"),
        };

        let name = pkg.get("name").context("package has no name")?
            .as_str().context("package name is not a string")?
            .to_string();
        let version = pkg.get("version").context("package has no version")?
            .as_str().context("package version is not a string")?
            .to_string();

        Ok(Box::new(JavaScript { pkg, name, version, _client }))
    }
}
